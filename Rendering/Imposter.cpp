...
void drawImposterTree(GLuint* textures, glm::mat4 modelmat,int level, double shearX, double shearZ) {
  int j = Num_Boards/2;
  glm::mat4 shearXX = GetShearMatrixX(shearX), shearZZ = GetShearMatrixX(shearZ);
  for (int i = 0; i < Num_Boards; i++) {
    if(abs(j-i) >= level - 1){
      drawSquare(textures[i], Projection*View*modelmat*GetPositionMatrix(0,0,(i-j)*0.45/Num_Boards)*shearXX);
      drawSquare(textures[Num_Boards+i], Projection*View*modelmat*gen_rotation_deg(0,90,0)*GetPositionMatrix(0,0,(i-j)*0.45/Num_Boards)*shearZZ); 
    }
  }
}
...
struct Imposters {
  ...
  void render(GLuint* textures, double shearX, double shearZ) {
    for (int i=0; i<Num_Imposters; i++) {
      drawImposterTree(&textures[i*2*Num_Boards], positions[i]*orientations[i]*scales[i]*GetPositionMatrix(0.0,1.0f, 0.0),levels[i], shearX, shearZ);
    }
  }
};  
