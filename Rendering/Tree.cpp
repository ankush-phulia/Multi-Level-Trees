...
void drawObj(objfile& f, glm::mat4 ModelMatrix = glm::mat4(1.0f)) {
    ...
    GLuint LightID =
        glGetUniformLocation(programObj, "LightPosition_worldspace");
  ...
  public:
    ...
    Tree(glm::mat4 p, glm::mat4 o) {
        ...
        g.insert(
            ...
            0.12);
        g.insert("B",
                 "F[dFl[Bl]][eFl[lB]][cFlc[BlFl]][aFl[aFl[Bl]cal[Bl]]][bFl["
                 "bFlc[Bl]]l]",
                 0.12);
        g.insert("B",
                 "F[dFl[lB]][eFl[Bl]][aFl[Bl]a[FlBl]][cFl[cFlacl[lB]]][bFl["
                 "bFla[lB]]l]",
                 0.12);
        g.insert(
            "B",
    ...
    void render(objfile Leaf_Object, objfile Branch_Object) {
        ...
        for (auto ch : Tree_string) {
            if (ch == 'F') {
                glm::mat4 modelmat = position * orientation *
                                     position_stack.back() *
                                     rotation_stack.back() *
                ...
                position_stack.push_back(
                    position_stack[position_stack.size() - 1]);
                rotation_stack.push_back(
                    rotation_stack[rotation_stack.size() - 1]);
            }
            else if (ch == ']') {
                ...
                glm::mat4 translation =
                    position_stack.back() * rotation_stack.back();
                glm::mat4 modelmat =
                    position * orientation * translation * leafmat;
