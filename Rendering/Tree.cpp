...
public:
  ...
  Tree(glm::mat4 p,glm::mat4 o) {
    g.insert("B","F[dFl[Bl]][eFl[lB]][aFla[BlFl]][cFl[cFlacl[Bl]]][bFl[bFla[Bl]]l]", 0.12);
    g.insert("B","F[eFl[Bl]][dFl[lB]][bFlb[BlFl]][cFl[cFlbcl[Bl]]][aFl[aFlb[Bl]]l]", 0.12);
    g.insert("B","F[dFl[Bl]][eFl[lB]][cFlc[BlFl]][aFl[aFl[Bl]cal[Bl]]][bFl[bFlc[Bl]]l]", 0.12);
    g.insert("B","F[dFl[lB]][eFl[Bl]][aFl[Bl]a[FlBl]][cFl[cFlacl[lB]]][bFl[bFla[lB]]l]", 0.12);
    g.insert("B","F[dFl[Bl]][eFl[lB]][aFl[Bl]a[BlFl]][cFl[cFl[Bl]acl[Bl]]][bFl[bFl[Bl]a[Bl]]l]", 0.52);
    g.insert("a","a");
    g.insert("b","b");
    g.insert("c","c");
    g.insert("d","d");
    ...
    rotations['a'] = gen_rotation((15.0 + angle1)*DegreeToRadiun, (30.0 + angle1)*DegreeToRadiun, (-20.0 + angle1)*DegreeToRadiun);
    rotations['b'] = gen_rotation((20.0 + angle2)*DegreeToRadiun, (20.0 + angle2)*DegreeToRadiun, (10.0 + angle2)*DegreeToRadiun);
    rotations['c'] = gen_rotation((-15.0 + angle3)*DegreeToRadiun, (20.0 + angle3)*DegreeToRadiun, (20.0 + angle3)*DegreeToRadiun);
    rotations['d'] = gen_rotation((-10.0 + angle4)*DegreeToRadiun, (-10 + angle4)*DegreeToRadiun, (0.0 + angle4)*DegreeToRadiun);
    rotations['e'] = gen_rotation((-10.0 + angle5)*DegreeToRadiun, (20 + angle5)*DegreeToRadiun, (10.0 + angle5)*DegreeToRadiun);

    position = p;
    orientation = o;
    leafmat = GetScaleMatrix(0.5,0.5,0.5)*GetPositionMatrix(0.0,0.0,-2.175);
  };
  ...
  void render(objfile Leaf_Object, objfile Branch_Object) {
    ...
    for (auto ch: Tree_string) {
      if (ch == 'F') {
        glm::mat4 modelmat =  position*orientation* position_stack.back() * rotation_stack.back() * get_boneMatrix(position_stack.size()) * GetPositionMatrix(0,5,0);
        drawObj(Branch_Object, modelmat);
        position_stack[position_stack.size()-1][3][0] += 10.0*get_boneMatrix(position_stack.size())[1][1]*rotation_stack[rotation_stack.size()-1][1][0];
        position_stack[position_stack.size()-1][3][1] += 10.0*get_boneMatrix(position_stack.size())[1][1]*rotation_stack[rotation_stack.size()-1][1][1];
        position_stack[position_stack.size()-1][3][2] += 10.0*get_boneMatrix(position_stack.size())[1][1]*rotation_stack[rotation_stack.size()-1][1][2];
      } else if (ch=='[') {
      ...
      } else if (ch=='a' or ch=='b' or ch=='c' or ch=='d' or ch=='e') {
