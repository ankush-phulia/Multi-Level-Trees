...
//  //    rotations['a'] = gen_rotation(20.0*DEG2RAD, 60.0*DEG2RAD, -45.0*DEG2RAD);
//  //    rotations['b'] = gen_rotation(45.0*DEG2RAD, 45.0*DEG2RAD, 15.0*DEG2RAD);
//  //    rotations['c'] = gen_rotation(-25.0*DEG2RAD, -180.0*DEG2RAD, 45.0*DEG2RAD);
//  //    rotations['d'] = gen_rotation(0.0*DEG2RAD, 70.0*DEG2RAD, -90.0*DEG2RAD);
...
string expand(string base) {
	...
	for (int i = 0; i < strlen(base.c_str()); i++) {
		if (base[i] == 'B') {
			...
			if (rando > 0.2) {
				expanded += "F[dF[BlBl]][aFla[BlFl]][cFl[cFlac[Bl]]][bFl[bFa[Bl]]l]";
			}
			else {
				expanded += 'l';
			}		
